...
var DIFFICULTY = 1; // Set difficulty to play a perfect game.
...
function humanPlay(i) {
  ...
  // Start the timer if the game has not started.
  if (!gameStarted) {
    gameStarted = true;
    startTimer();
  }
...
function drawScore() {
  ...
  if (text) {
    ...
    // Stop the timer if the game has finished.
    stopTimer();
    const timer = document.getElementById('timer').innerText;
        const wins = document.getElementById('wins').innerText;
        const losses = document.getElementById('losses').innerText;
        const ties = document.getElementById('ties').innerText;

        fetch('/update_ticktack', {
            method: 'POST',
            headers: {
            'Content-Type': 'application/json'
            },
            body: JSON.stringify({ timer: timer, wins: wins, losses:losses, ties:ties })
        })
...
function reset() {
  ...
  // Do not reset timer variables to keep the timer paused
  gameStarted = false; // Reset game started flag
  timerPaused = false; // Allow timer to resume
}
