...
let moves = 0;  // Define a global variable to keep track of the number of moves.
let startTime = null;
...
function input(n) {
  ...
  if (selectedDisk) {
    ...
    incrementMoves();  // Increment the moves counter when a valid move is made.
    checkWin();
...
function checkWin() {
  ...
  fetch('/update_hanoi', {
      ...
      body: JSON.stringify({ moves: moves, timer: timer })
  })
...
// Add a function to increment the moves counter
