...
// Score of the game, calculated based on the original Tetris scoring system.
var score = 0;

// Level of the game, increases as more lines are cleared.
var level = 1;
...
function startGame() {
  ...
  level = 1; // Reset level to 1 at the start of the game
  recalculateSpeed();
  initDataGrid();
...
function fail() {
  const score = document.getElementById('score').innerText;
              ...
              fetch('/update_tetris', {
                  ...
                  body: JSON.stringify({ score: score, level: level, lines:lines })
              })
...
function updateScore(clearedLines) {
  var lineScores = {1: 40, 2: 100, 3: 300, 4: 1200};
  var scoreToAdd = lineScores[clearedLines] || 0;
  score += scoreToAdd * START_LEVEL; // Assuming START_LEVEL as the current level for simplicity.
  document.getElementById('score').textContent = score;

  // Increase level every 10 lines
  if (lines % 10 === 0 && lines !== 0) {
    level++;
    recalculateSpeed(); // Update the speed based on the new level
